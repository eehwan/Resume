...
### - Django를 이용한 웹페이지 만들기 中

<br>

#### MVC패턴에 대한 이해 부족

> 파이썬으로 간단한 웹페이지를 구동하고 나서 파이썬 프레임워크인 django를 이용하여 웹페이지를 만들어 보려고 시도했었다.

> 하지만 MVC패턴에 대한 기본적인 지식도 없이 튜토리얼을 따라 만드는 방식은 나에게 수박 겉핥기 같다는 느낌만 들 뿐, 아무런 이해도 시켜주지 못했다.

> 그래서 결국, 이 프로젝트는 중도 포기하고 nodeJS와 express를 이용한 차근차근 하나씩 구현해보는 방식으로 새로운 프로젝트를 시작하게 되었다.
...
>또한 작은 프로젝트를 진행하며 며칠 내지 몇 주 넘게 한 가지에 몰두하여 작업하고 마무리하여 끝마칠 때 드는 만족감이 이 일을 사랑하게 만들었고, 이 일을 업으로 하며 살아갈 수 있게 더 성장하고 싶다는 욕구의 원동력이 되었다.
